...
<div class="categories-list lg:w-1/2 w-full mx-auto">
 ...
 <div class="text-right">
   <span>
   <button class="bg-blue-500 px-2 py-1  text-white text-sm font-bold rounded mr-2" @click="router.push('/category/create')">Add Category</button>
   </span>
   ...
   <table class="border-collapse border border-slate-400 w-full text-black text-left">
        ...
        <tbody class="text-black">
            <tr v-for="(category, index) in categories" :key="index" class="text-center">
                ...
                <td class="text-left px-2 py-2 capitalize border border-slate-300 font-bold cursor-pointer" @click="gotoTodo(category)">
                    {{category.name}}
                 </td>
...
export default{
    setup(){
      ...
      const deleteCategory = (category, index) => {
        deleteDoc(doc(db, "users", userUid.value,  "categories", category.id));
        categories.value.splice(index,1);
      }

      const gotoTodo = (category) => {
        localStorage.setItem("categoryId", category.id);
        router.push('/todos')
      }
       
      return{categories,router,editCategory,deleteCategory, userUid, gotoTodo}
   }
  }
</script>
